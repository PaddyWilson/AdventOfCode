	...
	public class Day17 : BaseDay
	{
		...
		protected override string Solution2(string[] input)
		{
			...
			while (turnCount < maxTurns)
			{
				...
				//{
				//	threads[i] = new Thread(() => { ThreadProcess(cubes, ref temp, size, w); });
				//	threads[i].Start();
				//}
				...
				{
					int tempW = w;
					threads[w] = new Thread(() => { ThreadProcess(cubes, ref temp, size, tempW); });
					threads[w].Start();
				}//for w
		...
		private void ThreadProcess(bool[,,,] cubes, ref bool[,,,] tempCubes, int size, int w)
		{
			for (int x = 1; x < size - 1; x++)
			{
				for (int y = 1; y < size - 1; y++)
				{
					for (int z = 1; z < size - 1; z++)
					{
						...
						{
							tempCubes[x, y, z, w] = true;//stay true
						}
						else if (count == 3 && cubes[x, y, z, w] == false)
						{
							tempCubes[x, y, z, w] = true;
						}
						else
						{
							tempCubes[x, y, z, w] = false;
						}
		...
		private string Solution2NotThreaded(string[] input)
		{
			int size = 102;
			int offset = size / 2;
			bool[,,,] cubes = new bool[size, size, size, size];

			Dictionary<(int, int, int, int), bool> cube = new Dictionary<(int, int, int, int), bool>();
